...
const isImageIcon = computed(() => {
  return props.icon?.endsWith('.webp') || 
...
const iconPath = computed(() => {
  if (isImageIcon.value && props.icon) {
...
<template>
  ...
  >
    <strong class="title">
      <img v-if="icon && isImageIcon" :src="iconPath" class="icon" :alt="title" />
      <SvgIcon v-else-if="icon" :icon="icon" class="icon" />
      {{ title }}
    </strong>
